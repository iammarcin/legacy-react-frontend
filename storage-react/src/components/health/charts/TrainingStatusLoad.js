...
const TrainingStatusLoad = ({ index, data, isFullWidth, isMobile, isModalOpen, onChartClick }) => {
  ...
  const processData = useCallback(() => {
    const keys = ['daily_training_load_acute', 'monthly_load_anaerobic', 'monthly_load_aerobic_high', 'monthly_load_aerobic_low', 'vo2_max_precise_value'];
    const transformFunctions = [value => value, value => value, value => value, value => value, value => value];
    const colors = ['red', 'violet', 'orange', 'green_light', 'blue_dark'];
    const labels = ['Daily Load Acute', 'Monthly Anaerobic', 'Monthly Aerobic High', 'Monthly Aerobic Low', 'vo2max'];
    const chartTypes = ['line', 'line', 'line', 'line', 'line'];
    const yAxisIDs = ['y-left', 'y-left', 'y-left', 'y-left', 'y-left'];
    const isHidden = [false, false, false, false, true];
...
export default TrainingStatusLoad;